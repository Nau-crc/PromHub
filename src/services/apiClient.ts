...
function normalizeGuest(row: any): Guest {
  return {
    ...
    checkedClub: !!row.checkedClub,
    cancelledClub: !!row.cancelledClub,
    waitlisted: !!row.waitlisted,
    extraEventIds: row.extraEventIds ?? [],
...
function buildSyncPayload(snap: AppDataSnapshot) {
  return {
    ...
    guests: snap.guests.map((g) => ({
      ...
      checkedClub: g.checkedClub ?? false,
      cancelledClub: g.cancelledClub ?? false,
      waitlisted: g.waitlisted ?? false,
      extraEventIds: g.extraEventIds ?? [],
