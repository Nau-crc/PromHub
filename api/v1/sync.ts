...
// /api/v1/sync — one-shot snapshot upload. POST only.
// Re-keys local IDs to backend SERIAL ids and returns the mapping
// so the client can patch its references.

export default safe(async () => {
  const { db, schema } = await import('../_lib/db');
  const { resolveTenant } = await import('../_lib/tenancy');
  const { snapshotSchema } = await import('../_lib/validators');
